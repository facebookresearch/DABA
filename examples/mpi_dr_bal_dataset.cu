...
int main(int argc, char *argv[]) {
  ...
  option.trust_region_option.relative_function_decrease_tolerance = 1e-6;
  ...
  problem =
      std::make_shared<sfm::ba::DouglasRachfordSubproblem<sfm::kGPU, T, false>>(
  ...
  T rho = (400.0 * ba_dataset->Measurements().size() /
  ...
  for (int iter = 0; iter < max_iters; iter++) {
    ...
    time_comm += sfm::utils::Timer::tock(start_comm);
    problem->EvaluateSurrogateFunction(
        problem->m_extrinsics, problem->m_intrinsics,
        problem->m_points_consensus, T(0.0), problem->m_surrogate_f, cost);
    error = DifferenceNorm(problem->m_points, problem->m_points_consensus,
                           problem->m_stream);
    rho *= 1 + eta;
  }
  ...
  if (rank == 0) {
    ...
    outfile = "dr-" + trust_region_solver_info + "-" + robust_loss_info + "-" +
